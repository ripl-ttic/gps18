...
gps_t *gps_create(const char *portname)
{
	...
	g->fd = bot_serial_open(portname, 9600, 1);
	if (g->fd <= 0) {
		free(g);
		return NULL;
	}
...
loop:
	...
	int res = bot_fileutils_read_line_timeout(g->fd, buf, maxlen, TIMEOUT_MS);
	if (res == 0)
		return GPS_TIMEOUT;
...
int gps_write_command(gps_t *g, const char *cmd)
{
	...
	bot_fileutils_write_fully(g->fd, cmd, strlen((char*) cmd));
	bot_fileutils_write_fully(g->fd, cmdchk, 4);
...
int gps_command(gps_t *g, const char *cmd)
{
	...
	uint8_t rpy[GPS_MESSAGE_MAXLEN];
...
int gps_autonegotiate_baud(gps_t *g, int guessbaud)
{
	...
	while (1) {
		...
		bot_serial_setbaud(g->fd, baud);
		bot_fileutils_read_flush(g->fd);
